...
void z_append_safety_facts_json(ZBuf *buf, const ZSafetyFactsProfile *profile) {
  ...
  zbuf_append(buf, ",\"coverage\":\"checker-mir-direct-backends\",\"bounds\":{\"policy\":");
  safety_append_json_string(buf, profile ? profile->bounds_policy : "checked");
  zbuf_append(buf, ",\"staticDiagnostics\":true,\"runtimeTraps\":true,\"trapRuntime\":\"pay-as-used\",\"optimizerElision\":");
  zbuf_append(buf, profile && profile->optimizer_elision ? "true" : "false");
  zbuf_append(buf, ",\"unchecked\":false},\"overflow\":{\"policy\":");
  safety_append_json_string(buf, profile ? profile->overflow_policy : "profile-defined-trap");
  zbuf_append(buf, ",\"integerOverflow\":");
  safety_append_json_string(buf, profile ? profile->overflow_policy : "profile-defined-trap");
  zbuf_append(buf, ",\"unchecked\":false}");
