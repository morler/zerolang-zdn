...
assert.match(rootHelp, /zero graph view \[--json\] \[--zdn\] \[--out <file\.0>\] <program-graph-or-source>/);
assert.match(rootHelp, /zero graph size \[--json\] \[--zdn\] \[--target <target>\] --out <artifact> <program-graph-or-package>/);
assert.match(rootHelp, /zero graph patch \[--json\] \[--zdn\] \[--out <program-graph-artifact>\] <program-graph-or-source> \(<patch-file>\|--op <operation>\)/);
...
assert.match(zeroText(["graph", "view", graphRenamedPath]).stdout, /pub fn start\(world: World\) -> Void raises/);
...
assert.equal(zeroText(["graph", "view", "--out", graphPackageViewPath, "examples/systems-package"]).stdout, "");
...
assert.equal(zeroText(["graph", "check", graphPackageViewPath]).stdout, "program graph check ok\n");
...
assert(rowGraph.functions.some((fun) => fun.name === "main"));  assert.equal(rejected.body.diagnostics[0].expected, ".0 source file, zero.json, or package directory");
}
for (const args of [
...
for (const kind of ["cli", "lib", "package"]) {
  ...
  assert.match(zeroText(["fmt", "--check", project]).stdout, /fmt ok/);
  if (kind !== "lib") {
